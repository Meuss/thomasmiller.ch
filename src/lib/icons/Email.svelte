<svg width="25" height="16" viewBox="0 0 39 26" fill="none" xmlns="http://www.w3.org/2000/svg">
	<g clip-path="url(#clip0_202_18)">
		<path
			d="M23.4955 13.9113L20.0094 16.7316C19.7308 16.9514 19.3249 16.9514 19.0543 16.7316L15.5682 13.9189L1.32918 25.3818H37.6708L23.4955 13.9113Z"
		/>
		<path d="M0 1.35669V24.5251L14.3345 12.9409L0 1.35669Z" />
		<path d="M1.32918 0.5L19.5 15.185L37.6708 0.5H1.32918Z" />
		<path d="M24.6655 12.9409L39 24.5251V1.35669L24.6655 12.9409Z" />
	</g>
	<defs>
		<clipPath id="clip0_202_18">
			<rect width="39" height="25" transform="translate(0 0.5)" />
		</clipPath>
	</defs>
</svg>
